...
|------|---------|----------|---------|
| 多克莫斯特 | Docker（卷挂载+热重载） | Docker（构建镜像） | `pnpm dev` |
| 代理人 | Docker（卷挂载 + --reload） | Docker（构建镜像） | `uvicorn --reload` |
| Wiki | Docker（卷挂载+热重载） | Docker（VitePress 构建→nginx） | `pnpm docs:dev` |
| 雷迪斯 | Docker 容器 | Docker 容器 | Docker 容器或外部 |
...
- 卷挂载：整个项目目录 → `/app`，命名卷隔离 `/app/node_modules`
...
- 卷挂载：`wiki/` → `/app`，命名卷隔离 node_modules
...
| 文件 | 修改 |
|------|------|
| `wiki/docs/.vitepress/config.ts:24` | `port: 5175` → `端口：parseInt(process.env.WIKI_PORT \ |\| '5175')` |
